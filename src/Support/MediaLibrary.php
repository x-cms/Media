...
use Request;
use Storage;
use Xcms\Base\Support\Traits\Singleton;
...
class MediaLibrary
{
     ...
     * The list can be customized with media.storage.media.ignore configuration option.
     */
    ...
    {
        $this->storageFolder = self::validatePath(Config::get('media.storage.media.folder', 'media'), true);
        $this->storagePath = rtrim(Config::get('media.storage.media.path', '/storage/app/public/media'), '/');

        if (!starts_with($this->storagePath, ['//', 'http://', 'https://'])) {
            $this->storagePath = Request::getBasePath() . $this->storagePath;
        }

        $this->ignoreNames = Config::get('media.storage.media.ignore');
    ...
    public function listFolderContents($folder = '/', $sortBy = 'title', $filter = null, $ignoreFolders = false)
    {
        ...
        if (array_key_exists($fullFolderPath, $cached)) {
            ...
            Cache::put(
                ...
                Config::get('media.storage.media.ttl', 10)
            );
        }
    ...
    public static function validatePath($path, $normalizeOnly = false)
    {
        ...
        if (preg_match($regex, $path) !== 0 || strpos($path, '//') !== false) {
            throw new Exception('不合法的路径: \':path\'.', compact('path'));
        }
    ...
    protected function getMediaRelativePath($path)
    {
        ...
        throw new Exception(sprintf('Cannot convert Media Library path "%s" to a path relative to the Library root.', $path));
    }
    ...
    protected function getStorageDisk()
    {
        ...
        return $this->storageDisk = Storage::disk(
            Config::get('media.storage.media.disk', 'local')
        );
    }
