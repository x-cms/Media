...
$.oc.mediaManager={};
let Base=$.oc.foundation.base,BaseProto=Base.prototype;
let MediaManager=function(element,options){this.$el=$(element);
...
MediaManager.prototype.getSelectedItems=function(returnNotProcessed,allowRootItem){let items=this.$el.get(0).querySelectorAll('[data-type="media-item"].selected'),result=[];
if(!allowRootItem){let filteredItems=[];
...
return items;
for(let i=0,len=items.length;i<len;i++){let item=items[i],itemDetails={itemType:item.getAttribute('data-item-type'),path:item.getAttribute('data-path'),title:item.getAttribute('data-title'),documentType:item.getAttribute('data-document-type'),folder:item.getAttribute('data-folder'),publicUrl:item.getAttribute('data-public-url')};
...
MediaManager.prototype.changeView=function(view){let data={view:view,path:this.$el.find('[data-type="current-folder"]').val()};
this.execNavigationRequest('onChangeView',data)};
MediaManager.prototype.setFilter=function(filter){let data={filter:filter,path:this.$el.find('[data-type="current-folder"]').val()};
...
this.selectTimer=null};
MediaManager.prototype.selectItem=function(node,expandSelection){if(!expandSelection){let items=this.$el.get(0).querySelectorAll('[data-type="media-item"].selected');
for(let i=0,len=items.length;i<len;i++){items[i].setAttribute('class','')}
...
MediaManager.prototype.unselectRoot=function(){let rootItem=this.$el.get(0).querySelector('[data-type="media-item"][data-root].selected');
if(rootItem)
...
MediaManager.prototype.selectFirstItem=function(){let firstItem=this.itemListElement.querySelector('[data-type="media-item"]:first-child');
if(firstItem){this.selectItem(firstItem)}};
MediaManager.prototype.selectRelative=function(next,expandSelection){let currentSelection=this.getSelectedItems(true,true);
if(currentSelection.length==0){this.selectFirstItem();
return}
let itemToSelect=null;
if(next){let lastItem=currentSelection[currentSelection.length-1];
if(lastItem)
itemToSelect=lastItem.nextElementSibling}
else{let firstItem=currentSelection[0];
if(firstItem)
itemToSelect=firstItem.previousElementSibling}
if(itemToSelect)
this.selectItem(itemToSelect,expandSelection)};
MediaManager.prototype.gotoFolder=function(path,resetSearch){let data={path:path,resetSearch:resetSearch!==undefined?1:0};
...
this.updateScroll()};
MediaManager.prototype.refresh=function(){let data={path:this.$el.find('[data-type="current-folder"]').val(),clearCache:true};
this.execNavigationRequest('onGoToFolder',data)};
...
this.navigationAjax=element.request(''+handler,{data:data}).always(function(){$.oc.stripeLoadIndicator.hide()}).done(this.proxy(this.afterNavigate)).always(this.proxy(this.releaseNavigationAjax))};
...
MediaManager.prototype.toggleSidebar=function(ev){let isVisible=this.isPreviewSidebarVisible(),$sidebar=this.$el.find('[data-control="preview-sidebar"]'),$button=$(ev.target);
...
this.$form.request('onSetSidebarVisible',{data:{visible:(isVisible?0:1)}})};
MediaManager.prototype.updateSidebarMediaPreview=function(items){let previewPanel=this.sidebarPreviewElement,previewContainer=previewPanel.querySelector('[data-control="media-preview-container"]'),template='';
for(let i=0,len=previewContainer.children.length;i<len;i++){previewContainer.removeChild(previewContainer.children[i])}
if(items.length==1&&!items[0].hasAttribute('data-root')){let item=items[0],documentType=item.getAttribute('data-document-type');
...
let items=resetSidebar===undefined?this.$el.get(0).querySelectorAll('[data-type="media-item"].selected'):[],previewPanel=this.sidebarPreviewElement;
...
let item=items[0],lastModified=item.getAttribute('data-last-modified');
...
let folderNode=previewPanel.querySelector('[data-label="folder"]');
...
this.sidebarThumbnailAjax=null}
let sidebarThumbnail=this.sidebarPreviewElement.querySelector('[data-control="sidebar-thumbnail"]');
if(!sidebarThumbnail)
return;
let data={path:sidebarThumbnail.getAttribute('data-path'),lastModified:sidebarThumbnail.getAttribute('data-last-modified')};
this.sidebarThumbnailAjax=this.$form.request('onGetSidebarThumbnail',{data:data}).done(this.proxy(this.replaceSidebarPlaceholder)).always(this.proxy(this.releaseSidebarThumbnailAjax))};
...
let placeholders=this.itemListElement.querySelectorAll('[data-type="media-item"] div.image-placeholder');
...
MediaManager.prototype.handleThumbnailQueue=function(){let maxThumbnailQueueLength=2,maxThumbnailBatchLength=3;
if(this.activeThumbnailQueueLength>=maxThumbnailQueueLength)
return;
for(let i=this.activeThumbnailQueueLength;i<maxThumbnailQueueLength&&this.thumbnailQueue.length>0;i++){let batch=[];
for(let j=0;j<maxThumbnailBatchLength&&this.thumbnailQueue.length>0;j++)
...
MediaManager.prototype.handleThumbnailBatch=function(batch){let data={batch:batch};
for(let i=0,len=batch.length;i<len;i++){let placeholder=document.getElementById(batch[i].id);
if(placeholder)
placeholder.setAttribute('data-loading','true')}
let promise=this.$form.request('onGenerateThumbnails',{data:data});
...
return;
for(let i=0,len=response.generatedThumbnails.length;i<len;i++){let thumbnailInfo=response.generatedThumbnails[i];
if(!thumbnailInfo.id||!thumbnailInfo.markup)
continue;
let node=document.getElementById(thumbnailInfo.id);
if(!node)
continue;
let placeholderContainer=node.parentNode;
...
this.handleThumbnailQueue()};
MediaManager.prototype.getRelativePosition=function(element,pageX,pageY,startPosition){let absolutePosition=startPosition!==undefined?startPosition:$.oc.foundation.element.absolutePosition(element,true);
...
return;
let uploaderOptions={clickable:this.$el.find('[data-control="upload"]').get(0),url:this.options.url,paramName:'file_data',headers:{},createImageThumbnails:false};
...
let fileNumberLabel=this.$el.get(0).querySelector('[data-label="file-number-and-progress"]'),messageTemplate=fileNumberLabel.getAttribute('data-message-template'),fileNumber=this.dropzone.getUploadingFiles().length+this.dropzone.getQueuedFiles().length;
...
MediaManager.prototype.setUploadProgress=function(value){let progressBar=this.$el.get(0).querySelector('[data-control="upload-progress-bar"]');
...
this.hideUploadUi()};
MediaManager.prototype.updateUploadBar=function(templateName,classNames){let fileNumberLabel=this.$el.get(0).querySelector('[data-label="file-number-and-progress"]'),successTemplate=fileNumberLabel.getAttribute('data-'+templateName+'-template'),progressBar=this.$el.get(0).querySelector('[data-control="upload-progress-bar"]');
...
MediaManager.prototype.cropSelectedImage=function(callback){let selectedItems=this.getSelectedItems(true);
...
return}
let path=selectedItems[0].getAttribute('data-path');
...
this.searchTrackInputTimer=null};
MediaManager.prototype.updateSearchResults=function(){let $searchField=this.$el.find('[data-control="search"]'),data={search:$searchField.val()};
...
MediaManager.prototype.onSearchChanged=function(ev){let value=ev.currentTarget.value;
...
MediaManager.prototype.deleteItems=function(){let items=this.$el.get(0).querySelectorAll('[data-type="media-item"].selected');
...
return;
let items=this.$el.get(0).querySelectorAll('[data-type="media-item"].selected'),paths=[];
for(let i=0,len=items.length;i<len;i++){paths.push({'path':items[i].getAttribute('data-path'),'type':items[i].getAttribute('data-item-type')})}
let data={paths:paths};
$.oc.stripeLoadIndicator.show();
this.$form.request('onDeleteItem',{data:data}).always(function(){$.oc.stripeLoadIndicator.hide()}).done(this.proxy(this.afterNavigate))};
...
MediaManager.prototype.onNewFolderSubmit=function(ev){let data={name:$(ev.target).find('input[name=name]').val(),path:this.$el.find('[data-type="current-folder"]').val()};
$.oc.stripeLoadIndicator.show();
this.$form.request('onCreateFolder',{data:data}).always(function(){$.oc.stripeLoadIndicator.hide()}).done(this.proxy(this.folderCreated));
ev.preventDefault();
return false};
MediaManager.prototype.folderCreated=function(){this.$el.find('button[data-command="create-folder"]').popup('hide');
this.afterNavigate()};
MediaManager.prototype.moveItems=function(ev){let items=this.$el.get(0).querySelectorAll('[data-type="media-item"].selected');
if(!items.length){$.oc.alert(this.options.moveEmpty);
return}
let data={exclude:[],path:this.$el.find('[data-type="current-folder"]').val()};
for(let i=0,len=items.length;i<len;i++){let item=items[i],path=item.getAttribute('data-path');
if(item.getAttribute('data-item-type')=='folder')
data.exclude.push(path)}
$(ev.target).popup({handler:'onLoadMovePopup',extraData:data,zIndex:1200})};
...
MediaManager.prototype.onMoveItemsSubmit=function(ev){let items=this.$el.get(0).querySelectorAll('[data-type="media-item"].selected'),data={dest:$(ev.target).find('select[name=dest]').val(),originalPath:$(ev.target).find('input[name=originalPath]').val(),files:[],folders:[]};
for(let i=0,len=items.length;i<len;i++){let item=items[i],path=item.getAttribute('data-path');
if(item.getAttribute('data-item-type')=='folder')
...
$.oc.stripeLoadIndicator.show();
this.$form.request('onMoveItems',{data:data}).always(function(){$.oc.stripeLoadIndicator.hide()}).done(this.proxy(this.itemsMoved));
ev.preventDefault();
return false};
...
this.afterNavigate()};
MediaManager.prototype.onNavigate=function(ev){let $item=$(ev.target).closest('[data-type="media-item"]');
this.navigateToItem($item);
if($(ev.target).data('label')!='public-url')
return false};
MediaManager.prototype.onCommandClick=function(ev){let command=$(ev.currentTarget).data('command');
...
break;case'popup-command':let popupCommand=$(ev.currentTarget).data('popup-command');
...
let pagePosition=$.oc.foundation.event.pageCoordinates(ev),relativePosition=this.getRelativePosition(this.itemListElement,pagePosition.x,pagePosition.y,this.itemListPosition);
...
let items=this.itemListElement.querySelectorAll('[data-type="media-item"]:not([data-root])'),selectionPosition=$.oc.foundation.element.absolutePosition(this.selectionMarker,true);
for(let index=0,len=items.length;index<len;index++){let item=items[index],itemPosition=$.oc.foundation.element.absolutePosition(item,true);
...
this.selectionStarted=false};
MediaManager.prototype.onListMouseMove=function(ev){let pagePosition=$.oc.foundation.event.pageCoordinates(ev),relativePosition=this.getRelativePosition(this.itemListElement,pagePosition.x,pagePosition.y,this.itemListPosition);
let deltaX=relativePosition.x-this.selectionStartPoint.x,deltaY=relativePosition.y-this.selectionStartPoint.y;
...
MediaManager.prototype.onSortingChanged=function(ev){let data={sortBy:$(ev.target).val(),path:this.$el.find('[data-type="current-folder"]').val()};
this.execNavigationRequest('onSetSorting',data)};
MediaManager.prototype.onKeyDown=function(ev){let eventHandled=false;
switch(ev.which){case 13:let items=this.getSelectedItems(true,true);
if(items.length>0)
...
let old=$.fn.mediaManager;
$.fn.mediaManager=function(option){let args=Array.prototype.slice.call(arguments,1),result=undefined;
this.each(function(){let $this=$(this);
let data=$this.data('oc.mediaManager');
let options=$.extend({},MediaManager.DEFAULTS,$this.data(),typeof option=='object'&&option);
...
$.oc.mediaManager={};
let Base=$.oc.foundation.base,BaseProto=Base.prototype;
let MediaManagerImageCropPopup=function(path,options){this.$popupRootElement=null;
...
this.registerHandlers()};
MediaManagerImageCropPopup.prototype.show=function(){let data={path:this.path};
this.$popupRootElement.popup({extraData:data,size:'adaptive',adaptiveHeight:true,handler:'onLoadImageCropPopup',zIndex:1200})};
...
if(this.rulersVisible){let $cropToolRoot=this.$popupElement.find('[data-control=media-manager-crop-tool]');
...
return;
let $cropToolRoot=this.$popupElement.find('[data-control=media-manager-crop-tool]'),width=$cropToolRoot.data('image-width'),height=$cropToolRoot.data('image-height');
if(!width||!height)
return;
...
let $hRuler=$cropToolRoot.find('[data-control=h-ruler]'),$vRuler=$cropToolRoot.find('[data-control=v-ruler]'),hTicks=width/40+1,vTicks=height/40+1;
...
MediaManagerImageCropPopup.prototype.createRulerTicks=function($rulerElement,count){let list=document.createElement('ul');
for(let i=0;i<=count;i++){let li=document.createElement('li');
li.textContent=i*40;
list.appendChild(li)}
...
MediaManagerImageCropPopup.prototype.fixDimensionValue=function(value){let result=value.replace(/[^0-9]+/,'');
if(!result.length)
result=200;
if(result=='0')
result=1;
...
return;
let $widthInput=this.getWidthInput(),$heightInput=this.getHeightInput(),width=this.fixDimensionValue($widthInput.val()),height=this.fixDimensionValue($heightInput.val()),mode=this.getSelectionMode();
...
break}};
MediaManagerImageCropPopup.prototype.cropAndInsert=function(){let data={img:$(this.imageArea).find('img').attr('src'),selection:this.jCrop.tellSelect()};
$.oc.stripeLoadIndicator.show();
this.$popupElement.find('form').request('onCropImage',{data:data}).always(function(){$.oc.stripeLoadIndicator.hide()}).done(this.proxy(this.onImageCropped))};
...
MediaManagerImageCropPopup.prototype.onResizePopupHidden=function(ev,button,popup){let $popup=$(popup),$widthControl=$popup.find('input[name=width]'),$heightControl=$popup.find('input[name=height]');
$popup.off('.media','form');
$widthControl.off('.media');
$heightControl.off('.media')};
MediaManagerImageCropPopup.prototype.onResizeDimensionKeyUp=function(ev){let $target=$(ev.target),targetValue=this.fixDimensionValue($target.val()),otherDimensionName=$target.attr('name')=='width'?'height':'width',$otherInput=$target.closest('form').find('input[name='+otherDimensionName+']'),ratio=this.$popupElement.find('[data-control=original-ratio]').val(),value=otherDimensionName=='height'?targetValue/ratio:targetValue*ratio;
$otherInput.val(Math.round(value))};
MediaManagerImageCropPopup.prototype.onResizeDimensionChanged=function(ev){let $target=$(ev.target);
$target.val(this.fixDimensionValue($target.val()))};
MediaManagerImageCropPopup.prototype.onResizeSubmit=function(ev){let data={cropSessionKey:this.$popupElement.find('input[name=cropSessionKey]').val(),path:this.$popupElement.find('input[name=path]').val()};
$.oc.stripeLoadIndicator.show();
$(ev.target).request('onResizeImage',{data:data}).always(function(){$.oc.stripeLoadIndicator.hide()}).done(this.proxy(this.imageResized));
ev.preventDefault();
return false};
...
MediaManagerImageCropPopup.prototype.onPopupHidden=function(event,element,popup){this.$popupElement.find('form').request('onEndCroppingSession');
...
MediaManagerImageCropPopup.prototype.onSelectionModeChanged=function(){let mode=this.getSelectionMode(),$widthInput=this.getWidthInput(),$heightInput=this.getHeightInput();
...
this.applySelectionMode()};
MediaManagerImageCropPopup.prototype.onImageScroll=function(){let scrollTop=this.imageArea.scrollTop,scrollLeft=this.imageArea.scrollLeft;
...
MediaManagerImageCropPopup.prototype.onSizeInputChange=function(ev){let $target=$(ev.target);
$target.val(this.fixDimensionValue($target.val()));
this.applySelectionMode()};
MediaManagerImageCropPopup.prototype.onCommandClick=function(ev){let command=$(ev.currentTarget).data('command');
